...
import { createUserFactory } from '../../utils/factories/user.factory';
...
describe('UserService', () => {
  ...
  describe('getUserById', () => {
    it('should return user domain object if found', async () => {
      const rawUser = createUserFactory();
      (prisma.user.findUnique as jest.Mock).mockResolvedValue(rawUser);

      const result = await service.getUserById(rawUser.id);

      expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { id: rawUser.id } });
  ...
  describe('getAllUsers', () => {
    it('should return array of user domain objects', async () => {
      const rawUsers = [
        createUserFactory(),
        createUserFactory({ id: 2, firstname: 'Jane', lastname: 'Smith', email: 'jane@example.com', usertype: UserType.admin }),
      ];
  ...
  describe('saveUser', () => {
    ...
    it('should update existing user if id provided', async () => {
      ...
      const updatedUserRaw = createUserFactory({ id: 1, firstname: 'Test', lastname: 'User', email: 'test@example.com' });
    ...
    it('should create new user and send welcome email if user does not exist', async () => {
      ...
      const createdUserRaw = createUserFactory({ id: 2, firstname: '', lastname: null, email: 'new@example.com', password: null, token: 'token123' });
    ...
    it('should return error if user with email exists', async () => {
      const existingUser = createUserFactory({ id: 5, email: 'exists@example.com' });
      const userDto = { email: existingUser.email, password: 'pass1234', userType: 'user' };
  ...
  describe('setPassword', () => {
    ...
    it('should prevent reuse of old passwords', async () => {
      ...
      (prisma.passwordHistory.findMany as jest.Mock).mockResolvedValue([{ password: 'hashedOldPass' }]);
    ...
    it('should update password if not reused', async () => {
      const user = createUserFactory();
      (prisma.user.findFirst as jest.Mock).mockResolvedValue(user);
      (prisma.passwordHistory.findMany as jest.Mock).mockResolvedValue([{ password: 'oldhashedpass' }]);
  ...
  describe('verifyToken', () => {
    it('should return true status if user with token found', async () => {
      (prisma.user.findFirst as jest.Mock).mockResolvedValue(createUserFactory());
  ...
  describe('loginAuth', () => {
    it('should return error if user not found or inactive', async () => {
      ...
      (prisma.user.findUnique as jest.Mock).mockResolvedValue(createUserFactory({ active: false }));
    ...
    it('should return error if user password not set', async () => {
      (prisma.user.findUnique as jest.Mock).mockResolvedValue(createUserFactory({ password: null }));
      const res = await service.loginAuth({ email: 'a', password: 'p' }, {});
      expect(res).toEqual({ error: 'Password not set for this user' });
    });

    it('should return error if password does not match', async () => {
      (prisma.user.findUnique as jest.Mock).mockResolvedValue(createUserFactory());
      (bcrypt.compare as jest.Mock).mockResolvedValue(false);
      const res = await service.loginAuth({ email: 'a', password: 'p' }, {});
    ...
    it('should return user and is_expire flag', async () => {
      ...
      const rawUser = createUserFactory({ lastPasUpdate });
      (prisma.user.findUnique as jest.Mock).mockResolvedValue(rawUser);
      (bcrypt.compare as jest.Mock).mockResolvedValue(true);
