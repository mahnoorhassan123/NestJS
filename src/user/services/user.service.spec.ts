...
describe('UserService', () => {
  ...
  describe('saveUser', () => {
    ...
    it('should update existing user if id provided', async () => {
      ...
      const updatedUserRaw = createUserFactory({
        id: 1,
        firstname: 'Test',
        lastname: 'User',
        email: 'test@example.com',
    ...
    it('should create new user and send welcome email if user does not exist', async () => {
      ...
      const createdUserRaw = createUserFactory({
        id: 2,
        firstname: '',
        lastname: null,
        email: 'new@example.com',
      ...
      expect(mailHelper.sendWelcomeEmail).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('setPassword', () => {
    ...
    it('should prevent reuse of old passwords', async () => {
      ...
      expect(result).toEqual({
        status: false,
        msg: 'Password already used, try a new one',
      });
    });
  ...
  describe('forgotPassword', () => {
    ...
    it('should update token and send reset password email', async () => {
      ...
      (mailHelper.sendResetPasswordEmail as jest.Mock).mockResolvedValue(
        undefined,
      );
      ...
      expect(prisma.user.update).toHaveBeenCalled();
      expect(mailHelper.sendResetPasswordEmail).toHaveBeenCalledWith(
        expect.objectContaining({
          to: user.email,
  ...
  describe('loginAuth', () => {
    it('should return error if user not found or inactive', async () => {
      ...
      (prisma.user.findUnique as jest.Mock).mockResolvedValue(
        createUserFactory({ active: false }),
      );
    ...
    it('should return error if user password not set', async () => {
      (prisma.user.findUnique as jest.Mock).mockResolvedValue(
        createUserFactory({ password: null }),
      );
    ...
    it('should return user and is_expire flag', async () => {
      ...
      const result = await service.loginAuth(
        { email: rawUser.email, password: 'password' },
        {},
      );
