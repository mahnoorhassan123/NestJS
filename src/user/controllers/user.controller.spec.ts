...
describe('UserController', () => {
    ...
    it('should get a user by ID', async () => {
        const mockUser: UserEntity = {
            ...
            password: 'hashedpass',
            userType: 'user',
            orderExportColumns: undefined,
            lastPasUpdate: undefined,
            active: true,
            isBlocked: false,
            blockMailSent: false,
            token: undefined,
    ...
    it('should save a user', async () => {
        const data = { firstname: 'John', lastname: 'Doe',   email: 'john@example.com', password: 'Password123', userType: 'admin'};
        const result = { success: true };
        userService.saveUser.mockResolvedValue(result);

        await controller.saveUser(data as any, mockReq as Request, mockRes as Response);
    ...
    it('should handle forgot password', async () => {
        ...
        await controller.forgotPassword(data as any, mockReq as Request, mockRes as Response);
    ...
    it('should handle google auth', async () => {
        const googleUserData = { email: 'test@example.com' };
        const result = { success: true };
        userService.googleAuth.mockResolvedValue(result);

        await controller.googleAuth({ value: googleUserData }, mockReq as Request, mockRes as Response);

        expect(userService.googleAuth).toHaveBeenCalledWith(googleUserData, mockReq);
        expect(mockRes.send).toHaveBeenCalledWith(result);
    });
    ...
    it('should handle login auth', async () => {
        ...
        await controller.loginAuth(data as any, mockReq as Request, mockRes as Response);
