...
export class CustomerService {
  ...
  async getCustomers(
    ...
    const customers= prismaCustomers.map((customer)=>{return {...CustomerMapper.toDomain(customer)}});
    return {
      page,
      limit,
      total,
  ...
  async getCustomerById(id: number) {
    const prismaCustomer = await this.prisma.customer.findUnique({ where: { id } });
    if (!prismaCustomer) {
  ...
  async createCustomer(data: CreateCustomerDto) {
    ...
    const prismaCustomer=CustomerMapper.fromCreateCustomerDto(data);
    const customer = await this.prisma.customer.create({ data: prismaCustomer });
    return {"status":200,"message":"Customer Saved","data":{"fieldCount":0,"affectedRows":1,"insertId":33263,"serverStatus":2,"warningCount":1,"message":"","protocol41":true,"changedRows":0}};
  ...
  async updateCustomer(id: number, data: UpdateCustomerDto) {
    ...
    const customerData=CustomerMapper.fromUpdateCustomerDto(data);
